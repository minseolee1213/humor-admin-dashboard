...
const items: Array<{ href: string; label: string; description: string }> = [
  { href: '/admin/images', label: 'Images', description: 'Create, update, delete images' },
  { href: '/admin/captions', label: 'Captions', description: 'Read-only captions table' },
  { href: '/admin/caption-requests', label: 'Caption Requests', description: 'Read-only requests for captions' },
  { href: '/admin/humor-flavors', label: 'Humor Flavors', description: 'Read-only humor flavor list' },
  { href: '/admin/humor-flavor-steps', label: 'Humor Flavor Steps', description: 'Read-only steps and prompts per flavor' },
];

export default async function AdminContentLandingPage() {
  ...
  return (
    <AdminLayout userEmail={session?.user.email}>
      <div className="mb-8">
        ...
        <p className="text-base text-gray-600">Tools for managing content and the humor system.</p>
      </div>
